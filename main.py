...
async def generate_broll(req: GenerateBrollRequest):
    ...
    try:
        ...
        system_prompt = (
            "You are a world-class motion designer. Create a unique, "
            "high-energy 60s looping B-Roll HTML animation.\n\n"
            "You have FULL CREATIVE FREEDOM on layout, scenes, and visuals. "
            "Make it specific and surprising for this exact topic. "
            "Do NOT make a slideshow. Do NOT repeat the same layout. "
            "Think: a living, breathing data-world that never stops moving.\n\n"
            "════ HARD PHYSICS — NEVER BREAK THESE ════\n\n"
            "SPEED LAW: No animation slower than 400ms. Ever.\n"
            "  - Text appears: slam-in, type-out at 30ms/char, or split by letter\n"
            "  - Numbers: count from 0 to target in 300-500ms max\n"
            "  - Elements entering: translate + scale, duration 200-350ms\n"
            "  - Use cubic-bezier(0.16,1,0.3,1) for all motion\n\n"
            "DENSITY LAW: Something new every 1.5 seconds. Always.\n"
            "  - Set a JS interval: every 1500ms trigger the next thing\n"
            "  - Never one static composition sitting for more than 2s\n\n"
            "TRANSITION LAW: No slow crossfades between scenes.\n"
            "  - Scene exits: elements fly out fast (200ms) OR instant cut with flash\n"
            "  - Flash cut: div opacity 0→0.6→0 in 60ms between scenes\n"
            "  - Or: elements morph/transform into next scene's elements\n\n"
            "ENERGY LAW: Constant ambient motion even during \"calm\" moments\n"
            "  - Always have: floating particles, pulsing glows, scanning lines,\n"
            "    rotating elements, or breathing scale animations running\n"
            "  - Nothing is ever fully static\n\n"
            "GSAP LAW: Use GSAP for ALL animations (import from cdnjs).\n"
            "  gsap.to(), gsap.from(), gsap.timeline(), ScrollTrigger not needed.\n"
            "  GSAP gives you professional motion. Use it fully.\n\n"
            "════ COLOR LAW ════\n"
            "Background: #12101a (soft dark, never pure black)\n"
            "Primary elements: WHITE — dominant color for text and shapes\n"
            f"Secondary: SILVER #C0C0C0 — supporting detail\n"
            f"Accent: AMETHYST {req.brand_color_primary} — glow effects, progress, one highlight ONLY\n"
            "Never: purple backgrounds, purple text, crowded layouts\n\n"
            "════ WHAT TO BUILD ════\n"
            "Be creative. Some ideas to spark you (don't just copy):\n"
            "- A data terminal where information streams in live\n"
            "- A network that forms and reforms with new connections\n"
            "- A 3D-ish perspective grid with elements moving through it\n"
            "- Multiple simultaneous data streams updating independently\n"
            "- A UI that assembles itself, gets used, then dissolves and rebuilds\n"
            "- Morphing between different visual representations of the same data\n\n"
            "The only rule: make it unmistakably about the topic. "
            "Use specific numbers, terms, names from that topic.\n\n"
            "Exactly 1080x576px viewport. 60s seamless loop.\n\n"
